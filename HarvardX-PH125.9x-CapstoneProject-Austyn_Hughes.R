...
#Variance inflation factor (VIF) of ModelOne.
vif(ModelOne)
...
#Variance inflation factor (VIF) of ModelTwo.
vif(ModelTwo)
...
pytWL._ggp <- ggplot(data = ValData, aes(x = WL., y = pytWL.))+ geom_point()+labs(x = 'Actual Winning Percentage', y = 'Phythagorean Predicted Winning Percentage', title = 'Actual vs Phythagorean Predicted')+ geom_smooth(method = 'lm')
pytWL._ggp

#RMSE for the standard model pytWL and each model created for pWL.
rmse_pytWL. <- rmse(ValData$WL.,ValData$pytWL.)

#Printing RMSE for pytWL.
rmse_pytWL.
